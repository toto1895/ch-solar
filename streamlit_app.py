...
def home_page():
    ...
    fcst_file = fetch_files(conn, "icon-ch/all_models_ch_prod")
    ...
    with st.spinner("Downloading and processing capacity data..."):
        ...
        if chart_type == "Forecast Chart":
            st.info(f"Fetch : {pd.to_datetime(fcst_file[-1].split('/')[-1].replace('.parquet',''),format='%Y%m%d%H',utc=True).tz_convert('CET')}")
