...
def plot_timeseries_with_nowcast(df, time_col="timestamp", target_col="solar_nowcast"):
    ...
    ignore_cols = [time_col, target_col, 'solar_groundstations', 'FastCloud']

    for col in d.columns.drop(ignore_cols, errors="ignore"):
        fig.add_trace(go.Scatter(
            x=d[time_col],
            y=d[col],
            name=col,
    ...
    try:
        ...
        gs = d['solar_groundstations']
        gs_unusable = gs.isna().all() or (gs.fillna(0) == 0).all()
        if gs_unusable:
            d['solar_hybrid'] = d[target_col]
        else:
            d['solar_hybrid'] = (d[target_col] + gs) / 2
...
def home_page():
    ...
    with st.spinner("Downloading and processing capacity data..."):
        ...
        st.warning(f"Master data latest update {powerplants['BeginningOfOperation'].max()} -- Estim. number: {round(1.1*len(powerplants)):,} -- Estim. Capacity: {1.15*powerplants['TotalPower'].sum()/1000:,.2f} MW" )
        ...
        for ddt in pd.date_range(start=dt.strftime("%Y%m%d"), freq='D', periods=5):
            try:
                ...
                stationprod = stationprod.tz_localize(None).tz_localize('UTC')
