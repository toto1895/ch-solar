...
def get_latest_entsoe_file(bucket_name="oracle_predictions", folder_prefix="swiss_solar/solar_actual/switzerland"):
    """
    Gets the latest ENTSOE data file from the GCS bucket.
    """
    try:
        ...
        if not blobs:
            st.warning(f"No ENTSOE data files found in {bucket_name}/{folder_prefix}")
            return None, None
        ...
        for blob in blobs:
            ...
            if match:
                ...
                try:
                    ...
                    date_blobs.append((date, blob))
                except ValueError:
                    continue
        
        if not date_blobs:
            st.warning(f"No valid date patterns found in ENTSOE data files")
            return None, None
            
        # Find the latest date
        latest_date, latest_blob = max(date_blobs, key=lambda x: x[0])
        return latest_date, latest_blob
        
    except Exception as e:
        st.error(f"Error getting latest ENTSOE file: {e}")
        return None, None

def load_entsoe_data(start_date, end_date=None):
    """
    Load ENTSOE data for the given date range from GCS bucket.
    Returns a dataframe with ENTSOE data.
    """
    try:
        if end_date is None:
        ...
        date_pattern = r'(\d{4}-\d{2}-\d{2})\.parquet
...
def Home():
    ...
    try:
        ...
        # Load ENTSOE data for the same period
        st.info("Loading ENTSOE actual data...")
        entsoe_df = load_entsoe_data(start_date, end_date)
        
        if entsoe_df.empty:
            st.warning("No ENTSOE data found in storage. Trying direct API call...")
            entsoe_df = fetch_entsoe_api_data(start_date, end_date)
        ...
        if not entsoe_df.empty:
            ...
            st.warning("No ENTSOE data available for the selected period.")
